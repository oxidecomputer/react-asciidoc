...
const VALID_ENTITY_TAIL =
  /^(?:[a-zA-Z][a-zA-Z]+\d{0,2}|#\d{1,7}|#x[\da-fA-F]{1,6});/;
  ...
  switch (node.type) {
    ...
    case "image": {
      ...
      if (node.subtype === "icon") {
        ...
        // For icons, the title sits on the outer <span> (matches Ruby)
        if (iconType === "text") {
          const label = node.alt || node.target;
          return `<span${classAttr}${idAttr}${titleAttr}>[${label}&#93;</span>`;
        }
        if (iconType === "font") {
          return `<span${classAttr}${idAttr}${titleAttr}><i class="fa fa-${escapeAttr(node.target)}"></i></span>`;
        }
        return `<span${classAttr}${idAttr}${titleAttr}><img src="${target}" alt="${escapeAttr(alt)}"${widthHeight}></span>`;
      }
      const classes = ["image"];
      if (node.role) classes.push(node.role);
      const classAttr = ` class="${classes.join(" ")}"`;
      ...
      return `<span${classAttr}${idAttr}><img src="${target}" alt="${escapeAttr(alt)}"${widthHeight}${titleAttr}></span>`;
    }
    ...
    case "footnote": {
      ...
      const idAttr = node.id
        ? ` id="_footnote_${escapeAttr(node.id)}"`
        : "";
